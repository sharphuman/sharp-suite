...
def render_auth():
    ...
    * { font-family: 'Nunito', sans-serif !important; }
    .stTextInput > div > div > input { background: rgba(255,255,255,0.05) !important; border: 1px solid rgba(99,102,241,0.3) !important; border-radius: 10px !important; color: white !important; padding: 12px 16px !important; }
    .stButton > button { background: linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%) !important; color: white !important; border: none !important; border-radius: 10px !important; padding: 12px 24px !important; font-weight: 600 !important; }
    ...
    .stTabs [data-baseweb="tab"] { background: transparent !important; color: #9ca3af !important; border-radius: 8px !important; padding: 10px 24px !important; font-weight: 600 !important; }
    ...
    with c2:
        st.markdown("""<div style="text-align:center;padding:50px 0 40px;"><img src="https://sharphuman.com/logo1-3.png" style="width:80px;margin-bottom:20px;"><h1 style="color:white;margin:0 0 8px;font-size:2rem;">Sharp Suite</h1><p style="color:#9ca3af;">AI-Powered Recruiting Tools</p></div>""", unsafe_allow_html=True)
        ...
        with tab1:
            ...
            st.markdown("<div style='display:flex;align-items:center;margin:24px 0;'><div style='flex:1;height:1px;background:rgba(255,255,255,0.1);'></div><span style='padding:0 16px;color:#6b7280;'>or</span><div style='flex:1;height:1px;background:rgba(255,255,255,0.1);'></div></div>", unsafe_allow_html=True)
        ...
        with tab2:
            ...
            if st.button("🎉 Create Account", use_container_width=True):
                if not se or not sp: st.warning("Fill all fields")
                elif len(sp) < 6: st.warning("Password must be 6+ characters")
        ...
        st.markdown("<p style='text-align:center;color:#6b7280;margin-top:24px;'>Need help? <a href='mailto:support@sharphuman.com' style='color:#6366f1;'>support@sharphuman.com</a></p>", unsafe_allow_html=True)
...
.app-card { background: rgba(255,255,255,0.03); border: 1px solid rgba(99,102,241,0.2); border-radius: 16px; padding: 28px; height: 180px; transition: all 0.3s; text-decoration: none !important; display: block; }
...
with st.sidebar:
    st.markdown(f"<div style='padding:16px;background:rgba(99,102,241,0.1);border-radius:12px;border:1px solid rgba(99,102,241,0.2);'><p style='color:#9ca3af;margin:0 0 4px;font-size:0.75rem;'>Logged in as</p><p style='color:white;margin:0;font-weight:600;'>{get_user_email()}</p>{'<p style=\"color:#f59e0b;margin:4px 0 0;font-size:0.75rem;\">👑 GOD MODE</p>' if st.session_state.is_god else ''}</div>", unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    if st.button("🚪 Logout", use_container_width=True):
        st.session_state.authenticated = False; st.session_state.user = None; st.session_state.is_god = False; st.rerun()

st.markdown(f"<div style='display:flex;align-items:center;gap:16px;padding:24px 0;border-bottom:1px solid rgba(99,102,241,0.2);margin-bottom:32px;'><img src='https://sharphuman.com/logo1-3.png' style='width:50px;'><div><h1 style='margin:0;font-size:1.8rem;'>Sharp Suite</h1><p style='color:#9ca3af;margin:0;'>Welcome back, {get_user_email()}</p></div></div>", unsafe_allow_html=True)

st.markdown("<div style='background:linear-gradient(135deg,rgba(99,102,241,0.1),rgba(139,92,246,0.1));border:1px solid rgba(99,102,241,0.2);border-radius:16px;padding:32px;margin-bottom:32px;text-align:center;'><h2 style='margin:0 0 8px;color:white;'>Your AI Recruiting Toolkit 🚀</h2><p style='color:#9ca3af;margin:0;'>Select an app to get started</p></div>", unsafe_allow_html=True)
...
if st.session_state.is_god:
    st.markdown("---")
    st.markdown("### ⚙️ Admin")
    st.markdown('<a href="https://admin.sharphuman.com" target="_blank" class="app-card" style="max-width:300px;border-color:rgba(239,68,68,0.3);"><div class="app-icon">⚙️</div><div class="app-title">Sharp Admin</div><div class="app-desc">Users & analytics</div></a>', unsafe_allow_html=True)

st.markdown("---")
st.markdown("<p style='text-align:center;color:#6b7280;'>© 2024 Sharp Human • <a href='https://sharphuman.com' style='color:#6366f1;'>sharphuman.com</a></p>", unsafe_allow_html=True)

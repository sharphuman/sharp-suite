"""Sharp Interview - Questions & Analysis with Supabase Auth"""
...
def render_auth():
    ...
    with col2:
        st.markdown("""<div style="text-align:center;padding:40px 0;"><img src="https://sharphuman.com/logo1-3.png" style="width:70px;margin-bottom:16px;"><h1 style="color:white;">Sharp Interview</h1><p style="color:#9ca3af;">Questions & Analysis</p></div>""", unsafe_allow_html=True)
...
with tab1:
    ...
    jd = c1.text_area("Requirements",height=120)
    stage = c2.selectbox("Stage",["Phone Screen","Technical","Manager","Final"])
    duration = c2.select_slider("Duration",["15 min","30 min","45 min","60 min"],value="45 min")
    if st.button("⚡ Generate",type="primary",use_container_width=True):
        if title:
            with st.spinner("⚡..."): st.markdown(f'<div class="output-box">{call_claude(f"Generate {stage} interview questions for {title}, {duration}. JD:{jd}. Include opening, 10 core questions, closing, red flags.")}</div>',unsafe_allow_html=True)
...
with tab2:
    ...
    if st.button("🔍 Analyze",type="primary",use_container_width=True,key="a"):
        if transcript:
            with st.spinner("🔍..."): st.markdown(f'<div class="output-box">{call_claude(f"Analyze interview for {role}:\n{transcript}\nGreen flags, red flags, recommendation.")}</div>',unsafe_allow_html=True)

with tab3:
    sr = st.text_input("Role",key="sr")
    comps = st.text_area("Competencies",height=80)
    if st.button("📋 Generate",type="primary",use_container_width=True,key="s"):
        if sr:
            with st.spinner("📋..."): st.markdown(f'<div class="output-box"><pre>{call_claude(f"Create scorecard for {sr}. Competencies: {comps or 'Technical, Communication, Culture'}. 1-5 scale.")}</pre></div>',unsafe_allow_html=True)

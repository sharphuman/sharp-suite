...
def create_session(user_id, email):
    ...
    try: requests.post(f"{SUPABASE_URL}/rest/v1/sessions", headers={"apikey": SUPABASE_ANON_KEY, "Authorization": f"Bearer {SUPABASE_SERVICE_KEY}", "Content-Type": "application/json", "Prefer": "return=minimal"}, json={"user_id": user_id, "token": token, "ip_address": "unknown", "device_hash": "interview", "is_active": True, "expires_at": (datetime.utcnow() + timedelta(hours=24)).isoformat()}, timeout=10)
    except: pass
    return token
...
def init_session():
    defaults = [('authenticated', False), ('user', None), ('is_god', False), ('session_token', None),
...
def extract_text_from_file(uploaded_file):
    """Extract text with robust PDF and DOCX handling"""
    ...
    if file_type == 'txt':
        ...
        # Fallback: Extract from DOCX as ZIP (raw XML)
        try:
...
def evaluate_candidate(candidate_name, cv_text, jd_text, transcript, focus_areas):
    ...
    prompt = f"""You are an expert interview evaluator. Analyze this candidate's interview performance.
...
1. **PROOF vs CLAIMS**: A CLAIM is when a candidate says they did something. PROOF is when they demonstrate HOW with specific details, examples, or technical depth. Score based on PROOF, not claims.

2. **CV Verification**: Check if claims in the CV are supported by evidence in the interview.

3. **Red Flags**: Note any evasive answers, inconsistencies, or gaps.
...
{{
    ...
    "overall_score": <0-100>,
    "overall_summary": "<2-3 sentence summary of candidate>",
    ...
    "focus_areas": [
        {{
            ...
            "evidence": ["<specific quote or example from transcript>", "<another example>"],
            "assessment": "<2-3 sentence assessment with specific observations>"
        }}
    ],
    "cv_verification": {{
        "trust_score": <0-10>,
        "verified_claims": ["<claim that was backed up in interview>"],
        "unverified_claims": ["<claim from CV not demonstrated>"],
        "inconsistencies": ["<any contradictions between CV and interview>"]
    }},
    "interview_quality": {{
    ...
    "questions_for_next_round": ["<question to probe deeper>", "<another question>"],
    "hiring_risk": "<description of risk if hired>",
    "not_hiring_risk": "<description of risk if not hired>"
}}
```

Be specific. Use exact quotes from the transcript as evidence. Be honest and constructive."""
...
def compare_candidates(candidates_data):
    ...
    prompt = f"""You are an expert hiring consultant. Compare these candidates and provide a final recommendation.
...
{{
    "comparison_table": {{
        "headers": ["Metric", {', '.join([f'"{c["candidate_name"]}"' for c in candidates_data])}],
        "rows": [
            ["Overall Score", ...scores...],
            ["Recommendation", ...recommendations...],
            ["Technical Skills", ...scores...],
            ["Communication", ...scores...],
            ["CV Trust Score", ...scores...],
    ...
    "head_to_head": [
        {{
            ...
            "analysis": "<why this candidate is stronger in this area>"
        }}
    ],
    "final_ranking": [
        {{
    ...
    "hiring_recommendation": {{
        "primary_choice": "<candidate name>",
        "primary_reasoning": "<detailed reasoning>",
        "backup_choice": "<candidate name or 'None'>",
        "backup_reasoning": "<reasoning>",
        "proceed_to_next_round": ["<candidate names>"],
        "do_not_proceed": ["<candidate names>"]
    }},
...
def display_candidate_result(result_data, candidate_name):
    """Display beautiful candidate evaluation results"""
    ...
    with col1:
        comm_score = iq.get('communication_score', 0)
...
.candidate-chip { display: inline-block; background: rgba(99,102,241,0.2); color: #a5b4fc; padding: 6px 14px; border-radius: 20px; margin: 4px; font-size: 13px; }
...
if not st.session_state.authenticated:
    ...
    with c2:
        st.markdown("""<div style="text-align:center;padding:60px 0 30px;"><img src="https://sharphuman.com/logo1-3.png" style="width:80px;margin-bottom:20px;"><h1 style="margin:0;">Sharp Interview</h1><p style="color:#9ca3af;">AI Interview Evaluation</p></div>""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    for key, label in [("portal", "🏠 Portal"), ("jd", "📝 JD Writer"), ("screen", "🔍 CV Screener"), ("interview", "🎯 Interview"), ("source", "🎣 Sourcing"), ("content", "✍️ Content"), ("sales", "💰 Sales"), ("reach", "🚀 Reach"), ("assistant", "🤖 Assistant")]:
        if key == "interview": st.markdown(f"<div style='background:linear-gradient(135deg,#6366f1,#8b5cf6);padding:10px 16px;border-radius:8px;text-align:center;margin:4px 0;color:white;font-weight:600;'>{label} ◀</div>", unsafe_allow_html=True)
...
# Header
st.markdown("""<div style="display:flex;align-items:center;gap:16px;padding:20px 0;border-bottom:1px solid rgba(99,102,241,0.2);margin-bottom:30px;"><img src="https://sharphuman.com/logo1-3.png" style="width:50px;"><div><h1 style="margin:0;font-size:28px;">Sharp Interview</h1><p style="color:#9ca3af;margin:0;">AI-Powered Interview Evaluation</p></div></div>""", unsafe_allow_html=True)

# Show evaluated candidates count
if st.session_state.candidates:
    st.markdown(f"**Evaluated Candidates:** {' '.join([f'<span class=\"candidate-chip\">{c[\"candidate_name\"]}</span>' for c in st.session_state.candidates])}", unsafe_allow_html=True)
    ...
    if st.session_state.comparison_result:
        try:
            ...
            if hr.get('backup_choice') and hr.get('backup_choice') != 'None':
                st.info(f"**Backup Choice:** {hr.get('backup_choice', 'N/A')}\n\n{hr.get('backup_reasoning', '')}")
            
            # Head to Head
            if comp.get('head_to_head'):
                st.markdown("### ⚔️ Head-to-Head Analysis")
                for h2h in comp.get('head_to_head', []):
                    st.markdown(f"**{h2h['area']}:** Winner: **{h2h['winner']}** — {h2h['analysis']}")
...
# STEP: RESULTS VIEW
    ...
    with col2:
        if len(st.session_state.candidates) < 4:
            if st.button(f"➕ Add Another Candidate ({len(st.session_state.candidates)}/4)"):
                st.session_state.current_step = 'input'
                st.session_state.current_candidate = None
                st.rerun()
    with col3:
    ...
    # Download
    st.markdown("---")
    st.download_button("📥 Download Evaluation", json.dumps(result, indent=2), f"{result.get('candidate_name', 'evaluation')}.json", use_container_width=True)
...
# STEP: INPUT VIEW
    ...
    # Candidate Name
    candidate_name = st.text_input("👤 Candidate Name", placeholder="e.g., John Smith", key=f"name_{candidate_num}")
    ...
    with col1:
        ...
        jd_method = st.radio("JD Input:", ["📝 Paste", "📁 Upload"], horizontal=True, key=f"jd_method_{candidate_num}")
        
        if jd_method == "📝 Paste":
            jd_text = st.text_area("Paste JD:", height=150, placeholder="Paste the job description here...", key=f"jd_text_{candidate_num}")
        else:
        ...
        cv_method = st.radio("CV Input:", ["📝 Paste", "📁 Upload"], horizontal=True, key=f"cv_method_{candidate_num}")
        
        if cv_method == "📝 Paste":
            cv_text = st.text_area("Paste CV:", height=150, placeholder="Paste the candidate's CV here...", key=f"cv_text_{candidate_num}")
        else:
            ...
            if cv_file:
                ...
                if cv_text and not cv_text.startswith("["):
                    st.success(f"✅ Loaded CV ({len(cv_text):,} chars)")
                else:
                    st.warning(cv_text)
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        trans_method = st.radio("Transcript Input:", ["📝 Paste", "📁 Upload"], horizontal=True, key=f"trans_method_{candidate_num}")
        
        if trans_method == "📝 Paste":
            transcript = st.text_area("Paste Transcript:", height=200, placeholder="Interviewer: Thanks for joining us today...\nCandidate: Thank you for having me...", key=f"trans_text_{candidate_num}")
        else:
            trans_file = st.file_uploader("Upload Transcript", type=['txt', 'pdf', 'docx', 'vtt', 'srt', 'mp3', 'wav', 'm4a', 'mp4'], key=f"trans_file_{candidate_num}")
            transcript = ""
            if trans_file:
                with st.spinner("Processing..."):
                    transcript = extract_text_from_file(trans_file)
                if transcript and not transcript.startswith("["):
                    st.success(f"✅ Loaded transcript ({len(transcript):,} chars)")
        ...
        focus_areas = st.multiselect("Select areas to evaluate:", FOCUS_AREAS, default=["Technical Skills", "Communication", "Problem Solving"], key=f"focus_{candidate_num}")
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    if st.button("🚀 Evaluate Candidate", type="primary", use_container_width=True):
        if not candidate_name:
            ...
            st.warning("Please provide a job description (min 50 characters)")
        elif not cv_text or len(cv_text) < 50:
            st.warning("Please provide the candidate's CV (min 50 characters)")
        elif not transcript or len(transcript) < 100:
            st.warning("Please provide the interview transcript (min 100 characters)")
            ...
            if not str(result).startswith("Error"):
                try:
                    ...
                    parsed['candidate_name'] = candidate_name  # Ensure name is set
...
with fb:
    with st.popover("💬 Feedback"):
        ...
        if st.button("Send", type="primary", use_container_width=True, key="fb_send"):
            if fm:
                if submit_feedback("interview", ft.split()[1].lower() if ft else "general", fm): st.success("Thanks! 🙏")

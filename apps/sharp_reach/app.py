...
def render_auth():
    ...
    with c2:
        st.markdown("""<div style="text-align:center;padding:40px 0;"><img src="https://sharphuman.com/logo1-3.png" style="width:70px;margin-bottom:16px;"><h1 style="color:white;">Sharp Reach</h1><p style="color:#9ca3af;">BD & Lead Outreach</p></div>""", unsafe_allow_html=True)
...
if st.button("🚀 Generate All Channels",type="primary",use_container_width=True):
    if name and company:
        context = f"Lead: {name}, {title} at {company} ({industry}). Pain: {pain}. Offering: {your_offer}"
        with st.spinner("🚀 Generating multi-channel outreach..."):
            c1,c2 = st.columns(2)
            with c1:
                st.markdown("#### 📧 Email Sequence")
                st.markdown(f'<div class="output-box">{call_claude(f"3-email BD sequence for {context}. Subject lines, bodies, timing.")}</div>',unsafe_allow_html=True)
                st.markdown("#### 📞 Phone Script")
                st.markdown(f'<div class="output-box">{call_claude(f"Cold call script for {context}. Opening, pitch, objection handling, close.")}</div>',unsafe_allow_html=True)
            with c2:
                st.markdown("#### 💼 LinkedIn")
                st.markdown(f'<div class="output-box">{call_claude(f"LinkedIn connection request (300 char) + follow-up InMail for {context}.")}</div>',unsafe_allow_html=True)
                st.markdown("#### 🎬 Video Script")
                st.markdown(f'<div class="output-box">{call_claude(f"60-second personalized video script for {context}.")}</div>',unsafe_allow_html=True)
            
            st.markdown("---")
            st.markdown("#### 🧠 AI Insights")
            st.markdown(f'<div class="output-box">{call_claude(f"For {context}: Best time to reach, predicted response rate, recommended channel priority, personalization tips.")}</div>',unsafe_allow_html=True)
    else:
        st.warning("Enter lead name and company")

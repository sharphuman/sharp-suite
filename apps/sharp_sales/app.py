"""Sharp Sales - Sales Analysis with Supabase Auth"""
...
def render_auth():
    ...
    with c2:
        st.markdown("""<div style="text-align:center;padding:40px 0;"><img src="https://sharphuman.com/logo1-3.png" style="width:70px;margin-bottom:16px;"><h1 style="color:white;">Sharp Sales</h1><p style="color:#9ca3af;">Sales Call Analysis</p></div>""", unsafe_allow_html=True)
...
st.markdown("""<div style="display:flex;align-items:center;gap:12px;padding:20px 0;border-bottom:1px solid rgba(99,102,241,0.2);margin-bottom:24px;"><img src="https://sharphuman.com/logo1-3.png" style="width:45px;"><div><h1 style="margin:0;">Sharp Sales</h1><p style="color:#9ca3af;margin:0;">Call Analysis • Deal Insights</p></div></div>""", unsafe_allow_html=True)

tab1,tab2 = st.tabs(["📞 Analyze Call","💎 Deal Insights"])

with tab1:
    transcript = st.text_area("Paste sales call transcript",height=300)
    c1,c2 = st.columns(2)
    deal_size = c1.text_input("Deal size","$50,000")
    stage = c2.selectbox("Stage",["Discovery","Demo","Proposal","Negotiation","Closing"])
    if st.button("🔍 Analyze",type="primary",use_container_width=True):
        if transcript:
            with st.spinner("🔍..."): st.markdown(f'<div class="output-box">{call_claude(f"Analyze sales call. Stage:{stage}. Deal:{deal_size}. Transcript:{transcript}. Include: summary, buying signals, objections, next steps, coaching tips, win probability.")}</div>',unsafe_allow_html=True)

with tab2:
    c1,c2 = st.columns(2)
    company = c1.text_input("Company")
    product = c1.text_input("Your product/service")
    competitor = c2.text_input("Main competitor")
    objection = c2.text_area("Key objections",height=100)
    if st.button("💎 Get Insights",type="primary",use_container_width=True,key="d"):
        if company and product:
            with st.spinner("💎..."): st.markdown(f'<div class="output-box">{call_claude(f"Deal strategy for {product} selling to {company}. Competitor:{competitor}. Objections:{objection}. Include: value props, differentiation, objection handling, negotiation tactics.")}</div>',unsafe_allow_html=True)
